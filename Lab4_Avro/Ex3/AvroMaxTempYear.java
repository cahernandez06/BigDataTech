...
public class AvroMaxTempYear extends Configured implements Tool
{
	...
	public static class AvroMapper extends Mapper<LongWritable, Text, AvroKey<GenericRecord>, NullWritable>
	{
		...
		Map<Integer,Float> mapTemperature = new HashMap<Integer,Float>();
		Map<Integer,String> mapIdStation = new HashMap<Integer,String>();
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
		{
			...
			if (utils.isValidTemperature())
			{
				...
				float temp = utils.getAirTemperature();
				int year = utils.getYearInt();
				
				if((mapTemperature.get(year) == null) || (temp > mapTemperature.get(year))){
					mapTemperature.put(year, temp);
					mapIdStation.put(year, stationId);
				} 						
			}
		}

		protected void cleanup(Context context) throws IOException, InterruptedException{	
			for(Integer y : mapTemperature.keySet()){
				record.put("stationId", mapIdStation.get(y));
				record.put("temperature", mapTemperature.get(y));
				record.put("year", y);				
